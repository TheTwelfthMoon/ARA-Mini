...
const int EEPROM_SIZE = 16;         // Размер EEPROM для хранения имени сети
        ...
        <div class="toggles">
            <div style="display: flex; align-items: center;">
                ...
                <div>
                    ...
                    <div class="toggles-container" style="width: 96px; margin: 0 auto 20px; ">
                        <div>1</div>
                        <div>2</div>
                    </div>
                </div>
        ...
        <div class="toggles">
            <div style="display: flex; align-items: center; ">
                ...
                <div>
                    <div class="toggles-container">
                        <div class="tri-state-toggle">
                            <input class="button toggle-switch" type="radio" name="toggleFlightSwitch" data-id="0"/>
                            <input class="button toggle-switch" type="radio" name="toggleFlightSwitch" data-id="1"/>
                        </div>
                    </div>
                    <div class="toggles-container" style="width: 96px; margin: 0 auto 20px; font-size: 9pt">
                        <div>ANGLE</div>
                        <div>ACRO</div>
                    </div>
                </div>
...
<script>
    ...
    let allReadings = [0, 0, 0, 0, 0, 0, 0, 0] // [armedButton, navState, LX, LY, RX, RY, flightState, programmingButton]
    ...
    toggleNavSwitchStates.forEach((el) => {
        ...
        el.addEventListener("click", (ev) => {
            allReadings[1] = +!allReadings[1]
            toggleNavSwitchStates.forEach((elem) => elem.style.opacity = "0")
            toggleNavSwitchStates.forEach((elem) => {
                if (+elem.dataset.id === allReadings[1]) elem.style.opacity = "1"
            })
        })
    })

    const toggleFlightSwitchStates = document.querySelectorAll('[name=toggleFlightSwitch]')
    toggleFlightSwitchStates.forEach((el) => {
        if (+el.dataset.id === allReadings[6]) el.style.opacity = "1"
        el.addEventListener("click", (ev) => {
            allReadings[6] = +!allReadings[6]
            toggleFlightSwitchStates.forEach((elem) => elem.style.opacity = "0")
            toggleFlightSwitchStates.forEach((elem) => {
                if (+elem.dataset.id === allReadings[6]) elem.style.opacity = "1"
            })
        })
    })
...
2. Нужно подняться на какую-то высоту :
Если нажата кнопка программирование, то выставляем navmod(2) и тротл 1700 (перевести в 0-100)

на вход float heigh - в метрах
heigh пересчитываем
...
4. запомнить высоту
выставляем тротл в 1500 (перевести в 0-100)

3. Разворот вправо.
yaw 1550 (перевести в [-100;100] ), 20 секунд
...
@ Артем
сразу режим альт холд (esp.nav_mode(1);), арм, от 0 до 1500, потом когда поднялся, активировать через делей пос холд. 


спросить на сколько нужно подняться. по времени тупо? или как-то регулировать высоту в методе
